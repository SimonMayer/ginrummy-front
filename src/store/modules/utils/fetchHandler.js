const actions = {
    async handleFetch({dispatch}, {key, timeout, forceFetch, fetchFunction, onSuccess, errorTitle}) {
        const response = {key};
        const shouldFetch = await dispatch('sessionState/fetch/shouldFetch', {key, timeout, forceFetch}, {root: true});
        ...
        dispatch('sessionState/fetch/recordAttempt', key, {root: true});
        ...
        try {
            ...
            dispatch('sessionState/fetch/recordFail', key, {root: true});
