        ...
        return await dispatch(
            'utils/fetchHandler/handleFetch',
            {
                ...
                onSuccess: async (playersData) => {
                    ...
                    const selfPlayerHandId = rootGetters['storage/players/self/getSelfPlayerRoundDataByRoundId'](roundId)?.hand?.hand_id;
                    return await dispatch('storage/hands/fetchHand', {handId: selfPlayerHandId}, {root: true});
                },
...
const getters = {
    ...
    isCurrentTurnForPlayer: (state, getters, rootState, rootGetters) => ({roundId, playerId}) => {
        ...
        const turn = rootGetters['storage/turns/turns/getTurnById'](currentTurnId);
